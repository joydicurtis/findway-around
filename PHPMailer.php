...
$login = 'i@toy-enjoy.com'; // замените test@domain.tld на адрес электронной почты, с которого производится отправка. Поскольку логин совпадает с адресом отправителя - данная переменная используется и как логин, и как адрес отправителя. 

$password = '33rjhjds!';  // Замените 'password' на пароль от почтового ящика, с которого производится отправка.
$to = 'findwayua@gmail.com';  // замените to@domain.tld на адрес электронной почты получателя письма.
$text="Привет!\n\n".$name." решил написать тебе: ".$message."\n\n";  // Содержимое отправляемого письма
...
$header.="From: =?UTF-8?Q?".str_replace("+","_",str_replace("%","=",urlencode('Findway Around Post')))."?= <$login>\r\n"; 
$header.="X-Mailer: Test script hosting Ukraine.com.ua \r\n"; 
$header.="Reply-To: =?UTF-8?Q?".str_replace("+","_",str_replace("%","=",urlencode('Findway Around Post')))."?= <$login>\r\n";
...
$code = substr(get_data($smtp_conn),0,3);
if($code != 250) {print "<div class='center'>Sending Error! You can get in touch with me via social networks</div>"; fclose($smtp_conn); exit;}

if($code == 250) {print "<div class='center'>Your message has been sent!</div>"  ;}
